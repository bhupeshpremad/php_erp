...
// Check if so_id is provided in the URL
...
try {
    // Fetch main SO data (from po_main table since SO is based on approved PO)
    ...
    // Handle case where SO is not found
    if (!$so_main) {
        die('SO not found or not approved');
    }

    // Fetch SO items (from po_items table)
    $stmt = $conn->prepare("SELECT * FROM po_items WHERE po_id = :so_id ORDER BY id ASC");
    $stmt->execute(['so_id' => $so_id]);
...
/**
 * PDF class extending FPDF to create a custom Sell Order document.
 */
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(0, 10, 'SELL ORDER', 0, 1, 'C');
        $this->SetFont('Helvetica', '', 12);
        $this->Cell(0, 8, 'SO Number: ' . $this->so_number, 0, 1, 'C');
        $this->Ln(2);
    }
    ...
    /**
     * Draws the main sell order form.
     * @param array $so_main Main sell order data.
     * @param array $so_items Array of sell order items.
     */
    function drawForm($so_main, $so_items) {
        $this->so_number = $so_main['sell_order_number'] ?? $so_main['po_number'];
        ...
        $this->Cell(34, 4, $so_main['sell_order_number'] ?? $so_main['po_number'], 0, 0);
        
        $this->SetFont('Arial', '', 8);
        $this->Cell(15, 4, "Dated", 0, 0);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(34, 4, date('d-M-y', strtotime($so_main['created_at'])), 0, 1);
        ...
        $this->SetXY(12, 78);
        $this->SetFont('Arial', 'B', 8);
        $this->Cell(0, 4, 'Customer (Bill to)', 0, 1);
        ...
        for ($j = count($so_items); $j < $maxRows; $j++) {
...
$pdf = new PDF();
$pdf->AddPage();
$pdf->drawForm($so_main, $so_items);
$pdf->Output('I', 'SO_' . preg_replace('/[^a-zA-Z0-9_-]/', '_', $so_main['sell_order_number'] ?? $so_main['po_number']) . '.pdf');
exit;
?>