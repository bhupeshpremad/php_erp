...
try {
    ...
    $stmt = $conn->prepare("SELECT id FROM supplier_quotations WHERE id = ? AND supplier_id = ?");
    $stmt->execute([$quotationId, $supplier_id]);
    if (!$stmt->fetch()) {
    ...
    $stmt = $conn->prepare("DELETE FROM supplier_quotations WHERE id = ?");
    $stmt->execute([$quotationId]);
