...
while (isset($_POST['item_name_' . $i])) {
    ...
    if (isset($_FILES[$fileInputName]) && $_FILES[$fileInputName]['error'] === UPLOAD_ERR_OK) {
        $uploadDir = ROOT_DIR_PATH . 'assets/images/upload/supplier_quotations/';
        if (!is_dir($uploadDir)) {
        ...
        $newFileName = 'supplier_quote_product_' . $supplier_id . '_' . time() . '_' . $i . '.' . $fileExtension;
...
try {
    ...
    if ($quotation_id) {
        ...
        $stmt = $conn->prepare("UPDATE supplier_quotations SET 
            quotation_number = ?, quotation_date = ?, customer_name = ?, customer_email = ?, customer_phone = ?, 
            delivery_term = ?, terms_of_delivery = ?, total_amount = ?, updated_at = NOW() WHERE id = ? AND supplier_id = ?");
        $stmt->execute([
            $quotation_number, $quotation_date, $customer_name, $customer_email, $customer_phone, 
            $delivery_term, $terms_of_delivery, $total_amount, $quotation_id, $supplier_id
        ]);
        ...
        $stmt = $conn->prepare("INSERT INTO supplier_quotations (
            supplier_id, rfq_reference, quotation_number, quotation_date, customer_name, customer_email, customer_phone,
            delivery_term, terms_of_delivery, total_amount, created_at, updated_at
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())");
        $stmt->execute([
            $supplier_id, 'N/A', $quotation_number, $quotation_date, $customer_name, $customer_email, $customer_phone,
