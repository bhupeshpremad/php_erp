...
if (!isset($_POST['supplier_name']) || !isset($_POST['product_type']) || !isset($_POST['product_name'])) {
    ...
    try {
        ...
        $stmt = $conn->prepare("
            SELECT pi.supplier_name, pi.product_type, pi.product_name, pi.invoice_number, pi.invoice_file, pi.billty_file 
            FROM purchase_items pi
            INNER JOIN purchase_main pm ON pi.purchase_main_id = pm.id
            WHERE pm.jci_number = ? 
            AND (pi.invoice_number IS NOT NULL AND pi.invoice_number != '')
            AND (pi.invoice_file IS NOT NULL AND pi.invoice_file != '')
        ");
...
try {
    ...
    $stmt = $conn->prepare("
        SELECT pi.invoice_number, pi.invoice_file, pi.billty_file 
        FROM purchase_items pi
        INNER JOIN purchase_main pm ON pi.purchase_main_id = pm.id
        WHERE pm.jci_number = ? 
    ...
    if ($result) {
        echo json_encode([
            ...
            'billty_file' => $result['billty_file']
        ]);
    } else {
