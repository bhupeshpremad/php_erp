...
$(document).ready(function() {
    ...
    document.querySelectorAll('.view-items-btn').forEach(function(btn) {
        btn.addEventListener('click', function() {
            ...
            const jciNumber = this.closest('tr').querySelector('td:nth-child(2)').textContent;
            document.getElementById('modalJciNumber').textContent = jciNumber;
                ...
                if (data.success && data.items.length > 0) {
                    data.items.forEach((item, idx) => {
                        ...
                        const productImage = item.product_image ? 
                            `<img src="${item.product_image}" alt="Product" style="width: 50px; height: 50px; object-fit: cover; cursor: pointer;" onclick="showImageModal('${item.product_image}', '${item.product_name}')" />` : 
...
<td>
    <a href="<?php echo BASE_URL; ?>modules/jci/download_job_card_pdf.php?job_card_number=${jciNumber}" class="btn btn-sm btn-success" target="_blank" style="margin-left: 5px;">Download PDF</a>
                    ...
                    tbody.innerHTML = `<tr><td colspan="13" class="text-center">${data.message || 'No details found.'}</td></tr>`;
                }
                new bootstrap.Modal(document.getElementById('itemDetailsModal')).show();
            })
            .catch(error => {
                const tbody = document.querySelector('#itemDetailsTable tbody');
                tbody.innerHTML = `<tr><td colspan="13" class="text-center text-danger">Error: ${error.message}</td></tr>`;
                new bootstrap.Modal(document.getElementById('itemDetailsModal')).show();
