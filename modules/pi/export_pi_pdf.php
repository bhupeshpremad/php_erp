<?php
    ...
    if (!defined('BASE_URL')) {
        ...
        define('BASE_URL', str_replace('/modules/pi/', '/', $base_url));
    }
    ...
    try {
        ...
        // Fetch PI details from the database
        ...
        // Fetch quotation details using the quotation_number from PI
        $stmt2 = $conn->prepare("SELECT * FROM quotations WHERE quotation_number = ?");
        $stmt2->execute([$pi['quotation_number']]);
        $quotation = $stmt2->fetch(PDO::FETCH_ASSOC);
        if (!$quotation) {
            exit('Quotation not found for this PI');
        }

        // Fetch product details for the quotation
        $stmt3 = $conn->prepare("SELECT * FROM quotation_products WHERE quotation_id = ?");
        $stmt3->execute([$quotation['id']]);
        ...
        // Header section with seller, buyer, and PI details
        $html .= '<table class="header-table">
            <tr>
                <th style="width: 33%;">Seller Details</th>
                <th style="width: 33%;">Buyer Details</th>
                <th style="width: 34%;">PI Details</th>
            </tr>
            <tr>
                <td>
                ...
                <td>
                    <strong>Payment Terms:</strong> ' . htmlspecialchars($pi['delivery_term'] ?? '60 Days') . '<br>
                    <strong>Terms of Delivery:</strong> ' . htmlspecialchars($pi['terms_of_delivery'] ?? 'FOB') . '<br>
                    <strong>Payment Term:</strong> ' . htmlspecialchars($pi['payment_term'] ?? '30% advance 70% on DOCS') . '<br>
                    <strong>Inspection:</strong> Internal<br>
                    <strong>PI Number:</strong> ' . htmlspecialchars($pi['pi_number'] ?? 'N/A') . '<br>
                    <strong>Date of PI Raised:</strong> ' . htmlspecialchars($pi['date_of_pi_raised'] ?? 'N/A') . '<br>
                    <strong>Sample Approval Date:</strong>
                </td>
            </tr>
        </table>';
        ...
        header('Content-Disposition: attachment; filename="pi_' . ($pi['pi_number'] ?? $piId) . '.pdf"');
        ...
        file_put_contents(__DIR__ . '/export_pi_pdf_error.log', date('Y-m-d H:i:s') . ' - ' . $e->getMessage() . "\n", FILE_APPEND);
