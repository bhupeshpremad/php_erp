...
if (isset($_GET['jci_number'])) {
    ...
    try {
        ...
        if (!empty($purchase_main_ids)) {
            ...
            $stmt_approved_items = $conn->prepare("SELECT id, supplier_name, product_type, product_name as item_name, assigned_quantity as quantity, price, total as item_amount, invoice_number, date as invoice_date, amount as invoice_amount, item_approval_status FROM purchase_items WHERE purchase_main_id IN ($placeholders) AND item_approval_status = 'approved'");
            ...
            foreach ($all_approved_purchase_items as $item) {
                ...
                $grouped_suppliers[$supplier_key]['items'][] = [
                    ...
                    'item_approval_status' => $item['item_approval_status'] ?? 'pending' // Include approval status
                ];
        ...
        foreach ($existing_payments as &$payment) {
            ...
            if ($payment['payment_category'] === 'Supplier') {
                $stmt_supplier_name = $conn->prepare("SELECT pi.supplier_name FROM purchase_items pi JOIN purchase_main pm ON pi.purchase_main_id = pm.id WHERE pm.jci_number = ? AND pi.invoice_number = ? AND pi.item_approval_status = 'approved' LIMIT 1");
        ...
        $stmt_invoice = $conn->prepare("SELECT DISTINCT pi.invoice_number, pi.date as invoice_date FROM purchase_items pi JOIN purchase_main pm ON pi.purchase_main_id = pm.id WHERE pm.jci_number = ? AND pi.item_approval_status = 'approved' LIMIT 1");
