...
$(document).ready(function() {
                ...
                if (response.success) {
                    ...
                    if (response.payments.length > 0) {
                        response.payments.forEach(function(payment, index) {
                            var paymentStatus = '';
                            var totalPurchaseItems = parseInt(payment.total_approved_purchase_items) || 0;
                            var paidPurchaseItems = parseInt(payment.paid_purchase_items) || 0;
                            
                            var totalAmount = parseFloat(payment.total_purchase_items_amount || 0) + parseFloat(payment.total_jci_items_amount || 0);
                            var paidAmount = parseFloat(payment.total_paid_amount || 0);

                            if (totalPurchaseItems === 0) {
                                paymentStatus = '<span class="badge badge-secondary">No Purchase Items</span>';
                            } else if (paidPurchaseItems >= totalPurchaseItems && paidPurchaseItems > 0) {
                                paymentStatus = '<span class="badge badge-success">Completed</span>';
                            } else if (paidPurchaseItems > 0) {
                                paymentStatus = '<span class="badge badge-info">Partial (' + paidPurchaseItems + '/' + totalPurchaseItems + ')</span>';
                            } else {
                            ...
                            rows += '<td>' + totalAmount.toFixed(2) + '</td>';
                            rows += '<td>' + paidAmount.toFixed(2) + '</td>';
                            rows += '<td>' + paymentStatus + '</td>';
                            rows += '<td><button class="btn btn-sm btn-outline-primary" onclick="downloadPaymentReport(' + (payment.payment_id || 0) + ')">Report</button></td>';
                            rows += '<td>';
                            rows += '<a href="add.php?id=' + (payment.payment_id || 0) + '" class="btn btn-primary btn-sm mr-1">Edit</a>';
                            rows += '<button class="btn btn-info btn-sm view-payment-details-btn" data-jci-number="' + (payment.jci_number || '') + '">View Details</button>';
                            rows += '</td>';
                            rows += '</tr>';
                        });
                    } else {
    ...
    $(document).on('click', '.view-payment-details-btn', function() {
        var jciNumber = $(this).data('jci-number'); // Get JCI number instead of payment ID
        ...
        $.ajax({
            ...
            type: 'GET',
            data: { jci_number: jciNumber }, // Pass JCI number
            dataType: 'json',
            success: function(response) {
                var totalAmount = 0;
                if (response.success && response.payment_details && response.payment_details.length > 0) {
                    response.payment_details.forEach(function(detail) {
                        ...
                        // Assuming invoice_image and builty_image are returned from the backend in detail object
                        ...
                        var row = '<tr>' +
                            ...
                            '<td>' + supplierInfo + (detail.invoice_number ? ' (Invoice: ' + detail.invoice_number + ')' : '') + '</td>' + // Add invoice number for suppliers
                            ...
                            '<td>' + builtyImage + '</td>' +
                            '<td>' + paymentStatus + '</td>' +
                            '<td><button class="btn btn-sm btn-outline-primary" onclick="downloadPaymentReport(' + (detail.payment_id || 0) + ')">Download Report</button></td>' +
                            '</tr>';
